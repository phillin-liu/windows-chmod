...
class ACLManager:
    ...
    def get_current_perms(filepath: str) -> Tuple[Dict[str, int], int]:
        ...
        try:
            ...
            for line in result.stdout.split("\n"):
                ...
                if norm_path and norm_path in line:
                    line = line.replace(norm_path, "", 1).strip()
                elif filepath in line:
                    line = line.replace(filepath, "", 1).strip()
    ...
    def set_permissions(filepath: str, perms: Dict[str, int],
        ...
        if grant_args:
            cmd_grant = ["icacls", filepath, "/grant"]
        ...
        if special and not quiet:
            ...
            if special_names:
                print(f"WARNING:Windows-chmod cannot set pecial bit(s) ({', '.join(special_names)}) "
                      f"set but have no direct Windows equivalent.", file=sys.stderr)
